function showpass(show) {
  if (show.className == "fas fa-eye-slash") {
    show.className = "fas fa-eye";
    show.previousElementSibling.type = "text";
  } else {
    show.className = "fas fa-eye-slash";
...
function checkinputpass(a){
  ...
  if (!filter.test(a.value)) {
    a.nextElementSibling.nextElementSibling.innerText="Password không hợp lệ!";
  } else {
...
function checkinputrepass(a){
  ...
  } else if(a.parentElement.parentElement.previousElementSibling.lastElementChild.firstElementChild.value!=a.value){
    a.nextElementSibling.nextElementSibling.innerText="Password không trùng khớp!";
    document.getElementsByClassName('btn-submit')[0].disabled=true;
  } else {
    a.nextElementSibling.nextElementSibling.innerText="";
